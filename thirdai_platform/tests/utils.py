...
def upload_model(client, access_token, name, access):
    ...
    assert (
        res.status_code == 200
    ), f"Failed to upload token: {res.status_code} - {res.text}"
    ...
    assert (
        res.status_code == 200
    ), f"Failed to upload chunk 1: {res.status_code} - {res.text}"
    ...
    assert (
        res.status_code == 200
    ), f"Failed to upload chunk 2: {res.status_code} - {res.text}"
    ...
    assert (
        res.status_code == 200
    ), f"Failed to commit upload: {res.status_code} - {res.text}"
