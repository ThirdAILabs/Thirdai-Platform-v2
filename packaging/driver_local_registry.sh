...
if [ "$VERBOSE" -eq 1 ]; then
    ...
    ansible-playbook playbooks/test_deploy.yml --extra-vars "config_path=$CONFIG_PATH qwen_model_path=$QWEN_MODEL_FULL_PATH docker_images=$DOCKER_IMAGES_PATH" -vvvv
else
    ansible-playbook playbooks/test_deploy.yml --extra-vars "config_path=$CONFIG_PATH qwen_model_path=$QWEN_MODEL_FULL_PATH docker_images=$DOCKER_IMAGES_PATH"
fi
