...
class TokenDataFactory(DataFactory):
    ...
    def get_fake_tag_values(self, tag: str, num_samples: int):
        # Generates num_samples number of fake values for a tag using faker
    ...
    def get_complete_tag_examples(
        ...
        # Generates fake values for a tag using faker or llm
        # Fake values generated by the LLM also takes into account the domain
        # and examples to generate new values.
    ...
    def get_templatized_examples(self, tags: List[str], k: int = 2):
        # Generates k templatized sentences for the tags. The templates can later be populated by fake values.
    ...
    def generate_data(
        ...
        dataset_config = self.process_chunks(
            arguments=arguments,
            num_sentences_to_generate=num_sentences_to_generate,
            sentences_generated=sentences_generated,
            tags=tags,
            complete_tag_examples=complete_tag_examples,
        )
        return dataset_config

    def process_prompt_chunks(
        self,
        arguments: List[str],
        num_sentences_to_generate: int,
        sentences_generated: int,
        tags,
        complete_tag_examples,
    ):
