...
export default function Page() {
  ...
  const handleRun = () => {
    ...
    predict(inputText).then((result) => {
      setPredictions(result.predicted_classes.map(([name, score]) => [name, Math.floor(score * 100)]));
    })
  }
  ...
  return (
    <div className="bg-muted" style={{width: "100%", display: "flex", justifyContent: "center", height: "100vh"}}>
      ...
      >
        ...
        <Box mt={4}>
          {
            predictions.map((prediction, index) => <Card key={index} style={{marginTop: '10px', width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
              <CardHeader>
              ...
              <div className='bg-muted' style={{display: 'flex', aspectRatio: 1, margin: '12px', justifyContent: 'center', alignItems: 'center', borderRadius: '5px', cursor: 'default', flexDirection: 'column', padding: '5px 10px 10px 10px'}}>
