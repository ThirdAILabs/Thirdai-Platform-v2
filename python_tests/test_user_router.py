...
def test_email_signup():
    response = requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={
            "username": "test1user",
...
def test_email_signup_duplicate_email():
    requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={"username": "user2", "email": "user3@mail.com", "password": "password"},
    )
    response = requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={"username": "user2", "email": "user3@mail.com", "password": "password"},
    ...
    print(
        "Test Email Signup Duplicate Email: ",
...
def test_email_signup_duplicate_username():
    requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={"username": "user41", "email": "user4m@mail.com", "password": "password"},
    )
    response = requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={"username": "user41", "email": "user2m@mail.com", "password": "password"},
    )
    print(
        "Test Email Signup Duplicate Username: ",
        "Passed" if response.status_code == 400 else "Failed",
        response.json(),
    )
...
def test_add_admin():
    ...
    requests.post(
        f"{BASE_URL}/api/user/email-signup-basic",
        json={"username": "user", "email": "user@mail.com", "password": "password"},
    )
    response = requests.post(
        f"{BASE_URL}/api/user/add-admin",
