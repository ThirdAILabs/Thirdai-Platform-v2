...
from ..llm.api_clients import OpenAILLM, CohereLLM, MockLLM, TokenCount
...
class TestCohereLLM(TestLLMBase):
    ...
    def test_completion_success(self):
        ...
        mock_completion.usage.billed_units = MagicMock(
            input_tokens=5,
            output_tokens=10
        )
...
@pytest.mark.parametrize("llm_class,api_key", [
    (OpenAILLM, "test_key"),
    (CohereLLM, "test_key")
])
def test_llm_initialization(llm_class, api_key):
    """Test that all LLM classes can be initialized"""
    with patch('openai.OpenAI' if llm_class == OpenAILLM else 'cohere.ClientV2') as mock_client:
