...
class UDTExtraOptions(BaseModel):
    ...
    def check_target_labels(cls, v, values):
        ...
        if sub_type == "token":
            ...
            for label in v:
                if len(label) == 0:
                    raise ValueError(f'target_labels cannot contain empty strings: "{label}" is invalid')
                if ' ' in label:
                    raise ValueError(f'target_labels cannot contain spaces: "{label}" is invalid')
        return v
    ...
    def check_n_target_classes(cls, v, values):
        ...
        if sub_type == "text":
            ...
            if v <= 0:
                raise ValueError(f'n_target_classes must be a positive integer: {v} is invalid')
        return v
