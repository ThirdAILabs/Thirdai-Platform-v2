...
class NDBRouter:
    ...
    def insert(
        ...
        if total_filesize > (max_filesize_mb * 1024 * 1024):
            return response(
                status_code=status.HTTP_400_BAD_REQUEST,
                message=f"Size of uploaded files exceeds maximum of {max_filesize}Mb for insertion endpoint on active deployment. Please use retraining api for updates of this size.",
            )
