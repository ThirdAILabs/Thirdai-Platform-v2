    ...
    def set_chat(self, **kwargs):
        ...
        with self.chat_instance_lock:
            ...
            try:
                ...
                key = kwargs.get("key") or self.config.options.get("genai_key", "random-key")   # openai.OpenAIError: The api_key client option must be set either by passing api_key to the client or by setting the OPENAI_API_KEY environment variable
