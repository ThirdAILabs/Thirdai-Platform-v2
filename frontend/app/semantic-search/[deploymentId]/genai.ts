...
export function genaiQuery(question: string, references: ReferenceInfo[], genaiPrompt: string) {
  ...
  const context = references
    .map((ref) => {
      ...
      if (lowerCaseSourceName.endsWith('.docx') || lowerCaseSourceName.endsWith('.pdf')) {
