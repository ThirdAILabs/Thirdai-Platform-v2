      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <iframe
              src={`${panelUrl}&panelId=1&from=now-24h&to=now&t=${Date.now()}`}
              width="425"
              height="300"
            ></iframe>
            <iframe
              src={`${panelUrl}&panelId=3&from=now-24h&to=now&t=${Date.now()}`}
              width="425"
              height="300"
            ></iframe>
            <iframe
              src={`${panelUrl}&panelId=2&from=now-24h&to=now&t=${Date.now()}`}
              width="425"
              height="300"
            ></iframe>
          </div>
