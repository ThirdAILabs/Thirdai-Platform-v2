...
export const PerformanceSummary: React.FC<PerformanceSummaryProps> = ({
  ...
  const calculateChanges = () => {
    const allLabels = new Set([...Object.keys(beforeMetrics), ...Object.keys(afterMetrics)]);
    ...
    return Array.from(allLabels)
      .map((label) => {
        ...
        const afterF1 = Number.isFinite(afterMetrics[label]?.fmeasure)
          ...
          : null;
        const change = beforeF1 !== null && afterF1 !== null ? afterF1 - beforeF1 : null;
  ...
  const overallBefore =
    validChanges.reduce((acc, curr) => acc + (curr.beforeF1 ?? 0), 0) / validChanges.length;
  const overallAfter =
    validChanges.reduce((acc, curr) => acc + (curr.afterF1 ?? 0), 0) / validChanges.length;
  ...
  return (
    <div className="space-y-4 mb-12">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Overview of F1 score changes across all labels</p>
      </div>
